...
本仓库对应论文中“外卖平台骑手行为与系统演化”实验的核心代码，包含两部分内容：

- `exp1` 到 `exp5`：论文中已经产出的实验结果数据及其分析脚本
- `simulation/` 与相关运行脚本：从原始 `SocialInvolution` 示例中整理出的、适合公开展示的轻量化模拟实现

当前仓库的目标不是完整复刻原始 MACE 工程，而是保留论文实验所需的核心流程，使代码结构更清晰、可多次运行、可直接分析。
...
- `exp1` 到 `exp5` 为已有实验结果数据；`simulation_results/` 为当前仓库重构后的模拟输出。
